...
def get_likelihoods(N, s_het, s_hom, g):
    anc_p = anc
    ...
    num_refresh = (num_gen - 5) // 20
    num_leftover_gen = (num_gen - 5) % 20
    num_growth_gen = num_gen - num_leftover_gen
    final_N = int(N * ((1 + g) ** num_growth_gen))
    to_return = np.zeros((end-start, final_N))
    ...
    for i in range(start, end):
        ...
        curr_gen = 5
        curr_N = int(N * ((1 + g) ** curr_gen))
        for _ in range(num_refresh):
            p_vec = fastDTWF._integrate_likelihood_constant_size(
                vec=p_vec,
                interval_pop_size=curr_N,
                interval_length=20,
                s_het=s_het,
                mu_0_to_1=mu,
                mu_1_to_0=zero,
        ...
        p_vec = fastDTWF._integrate_likelihood_constant_size(
            vec=p_vec,
            interval_pop_size=curr_N,
            interval_length=num_leftover_gen,
            s_het=s_het,
            mu_0_to_1=mu,
            mu_1_to_0=zero,
            dtwf_tv_sd=0.1,
...
xy = get_likelihoods(n_haploid_modern, het, hom, growth)
np.save(args.out, xy)
